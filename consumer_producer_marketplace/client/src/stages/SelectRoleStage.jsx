...
export function SelectRolesStage() {
  ...
  function handleSubmit() {// initializing variables here. Might be done server side in the future  
    ...
    if (player.get("role") === "producer") {
      ...
      // NOTE: stock 1 -> low stock 2 -> high
      if (player.get("stocklow") === undefined) {
        player.set("stocklow", 0);
      }
      if (player.get("stockhigh") === undefined) {
        player.set("stockhigh", 0);
      }
      if (player.get("capital") === undefined && player.get("round") === 1) {
