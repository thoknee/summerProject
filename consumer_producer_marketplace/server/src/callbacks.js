  ...
  await game.players.forEach(async (player) => {
    ...
    if (consumerAgent.strategy == "gullible") {
      ...
      if (soldStock == 0) {
        ...
        if(soldStock > 0){
          if (productAdQuality === "high" && productQuality === "high") {
              roundRating = 5; 
          ...
          consumerAgent.rating = roundRating;
    ...
    if (consumerAgent.strategy == "informed") {
      ...
      //Here is where we start adding our own code.
      //We have to check round two seperately from any round > 2 so that we don't get an out of bounds
      //exception if we try to access 2 rounds ago.
      ...
      //If the round before AND two rounds before are both not cheated, then the consumer will just buy 
      ...
      //If either of the last two rounds were considered "cheated", then the consumer
      //Will not buy anything at all.
